...
public class BrokerRequestHandler {
  ...
  private BrokerResponse processSingleTableBrokerRequest(@Nonnull BrokerRequest brokerRequest,
    ...
    int numServersResponded = responseMap.size();
    ...
    deserializeResponses(tableName, responseMap, -1, dataTableMap, processingExceptions);
    ...
    // Attach processing exceptions and number of servers queried/responded.
    ...
    if (brokerResponse.getExceptionsSize() > 0) {
      _brokerMetrics.addMeteredTableValue(tableName, BrokerMeter.PROCESSING_EXCEPTION_CAUGHT, 1);
    }
    if (numServersQueried > numServersResponded) {
      _brokerMetrics.addMeteredTableValue(tableName, BrokerMeter.PARTIAL_SERVERS_RESPONDED, 1);
    }
  ...
  private BrokerResponse processFederatedBrokerRequest(@Nonnull BrokerRequest originalBrokerRequest,
    ...
    Map<String, Map<ServerInstance, ByteBuf>> responseMaps = new HashMap<>(compositeFutureMap.size());
    List<ProcessingException> processingExceptions = new ArrayList<>();
    for (Entry<String, CompositeFuture<ServerInstance, ByteBuf>> entry : compositeFutureMap.entrySet()) {
      ...
      try {
        responseMap = compositeFuture.get();
        numServersResponded += responseMap.size();
        Map<String, Long> responseTimes = compositeFuture.getResponseTimes();
        scatterGatherStats.setResponseTimeMillis(responseTimes);
        responseMaps.put(tableName, responseMap);
      } catch (Exception e) {
    ...
    for (Entry<String, Map<ServerInstance, ByteBuf>> entry : responseMaps.entrySet()) {
      String tableName = entry.getKey();
      Map<ServerInstance, ByteBuf> responseMap = entry.getValue();
      deserializeResponses(tableName, responseMap, responseSequence, dataTableMap, processingExceptions);
      responseSequence++;
    }
    ...
    // Update broker metrics.
    if (brokerResponse.getExceptionsSize() > 0) {
      _brokerMetrics.addMeteredTableValue(originalTableName, BrokerMeter.PROCESSING_EXCEPTION_CAUGHT, 1);
    }
    if (numServersQueried > numServersResponded) {
      _brokerMetrics.addMeteredTableValue(originalTableName, BrokerMeter.PARTIAL_SERVERS_RESPONDED, 1);
    }
   ...
   */
  private void deserializeResponses(@Nonnull String tableName, @Nonnull Map<ServerInstance, ByteBuf> responseMap,
