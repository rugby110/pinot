...
public class AggregationPlanNode implements PlanNode {
  ...
  public Operator run() {
    ...
    for (int i = 0; i < numAggFuncs; i++) {
      ...
      aggrFuncContextArray[i] = AggregationFunctionContext.instantiate(_aggregationInfos.get(i), new AggregationFunctionVisitorBase() {
            @Override
            public void visit(FastHllAggregationFunction function) {
              function.init(_indexSegment.getSegmentMetadata().getHllLog2m());
            }
          });
    }
